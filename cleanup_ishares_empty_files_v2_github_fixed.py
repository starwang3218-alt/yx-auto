...
def looks_like_holdings_header(line: str) -> bool:
    upper = line.upper()
    return (
        line.startswith(HOLDINGS_HEADER_PREFIX)
        or ("TICKER" in upper and ("CUSIP" in upper or "ISIN" in upper or "ASSET CLASS" in upper))
...
def parse_date_from_dir_name(name: str) -> dt.date | None:
    """Support both old VPS raw folder YYYY-MM-DD and GitHub history raw folder YYYYMMDD."""
    for fmt in ("%Y%m%d", "%Y-%m-%d"):
        try:
...
def is_ishares_empty_template(path: Path) -> tuple[bool, str]:
    ...
    # 找 holdings 表头。不要只认一个固定前缀，因为不同 iShares CSV 列可能略有差异。
    header_idx = None
    for i, line in enumerate(non_empty):
        if looks_like_holdings_header(line):
            header_idx = i
            break
...
def main() -> int:
    ...
    parser.add_argument("--raw-root", default=None, help="可选：直接指定 raw 目录；优先级高于 --root/--vendor")
    ...
    root = Path(args.root)
    raw_root = Path(args.raw_root) if args.raw_root else root / "data" / "vendors" / args.vendor / "raw"
